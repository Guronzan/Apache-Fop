...
@Slf4j
 public class UnresolvedPageNumber extends TextArea implements Resolvable {
     ...
     *            the list of PageViewports associated with this ID
     ...
     public void resolveIDRef(final String id, final List<PageViewport> pages) {
         if (!this.resolved && this.pageIDRef.equals(id) && pages != null) {
             ...
             this.resolved = true;
             final int pageIndex = this.pageType ? 0 : pages.size() - 1;
             final PageViewport page = pages.get(pageIndex);
             // replace the text
             removeText();
             this.text = page.getPageNumberString();
             addWord(this.text, 0, getBidiLevel());
             // update ipd
             if (this.font != null) {
     ...
     * @return true if there is an UnresolvedArea descendant
     ...
     /**
      * Collection bidi inline runs. Override of @{link InlineParent}
     * implementation.
     *
      * N.B. [GA] without this override, the page-number-citation_writing_mode_rl
     * layout engine test will fail. It may be that the test needs to be updated
     * rather than using this override.
     * 
     * @param runs
