...
public abstract class AbstractBreaker {
    ...
    public static class PageBreakPosition extends LeafPosition {
        ...
        PageBreakPosition(final LayoutManager lm,
                final int breakIndex, // CSOK: ParameterNumber
     ...
     */
    protected abstract List<KnuthElement> getNextKnuthElements(
    ...
    public void doLayout(final int flowBPD, final boolean autoHeight) {
        ...
        while (hasMoreContent()) {
            ...
            for (int blockListIndex = 0; blockListIndex < this.blockLists
                    .size(); blockListIndex++) {
                blockList = this.blockLists.get(blockListIndex);
                ...
                if (Math.abs(alg.getIPDdifference()) > 1) {
                    ...
                    this.blockLists.clear();
                    nextSequenceStartsOn = getNextBlockListChangedIPD(childLC,
                            alg, effectiveList);
                    blockListIndex = -1;
                } else {
            ...
            if (startElementIndex <= endElementIndex) {
                ...
                if (pbp.difference != 0 && displayAlign == Constants.EN_CENTER) {
                    ...
                    while (effectiveListIterator.nextIndex() <= endElementIndex) {
                        final KnuthElement tempEl = effectiveListIterator
                                .next();
     ...
     *            LayoutContext to use
     * @param alg
     *            the pagebreaking algorithm
     * @param effectiveList
     *            the list of Knuth elements to be reused
     * @return the page on which the next content should appear after a hard
     *         break
     */
    private int getNextBlockListChangedIPD(final LayoutContext childLC,
            final PageBreakingAlgorithm alg, final BlockSequence effectiveList) {
        int nextSequenceStartsOn;
        final KnuthNode optimalBreak = alg.getBestNodeBeforeIPDChange();
        int positionIndex = optimalBreak.position;
        log.trace("IPD changes at index " + positionIndex);
        final KnuthElement elementAtBreak = alg.getElement(positionIndex);
        ...
        if (containsNonRestartableLM(positionAtBreak)) {
            ...
            final Iterator<KnuthElement> iter = effectiveList
                    .listIterator(positionIndex + 1);
            Position position = null;
            while (iter.hasNext()
                    && (position == null || containsNonRestartableLM(position))) {
                positionIndex++;
                final KnuthElement element = iter.next();
        ...
        if (positionAtBreak != null && positionAtBreak.getIndex() == -1) {
            /*
             * This is an indication that we are between two blocks (possibly
             * surrounded by another block), not inside a paragraph.
             */
            Position position;
            final Iterator<KnuthElement> iter = effectiveList
                    .listIterator(positionIndex + 1);
            do {
                final KnuthElement nextElement = iter.next();
            ...
            final LayoutManager surroundingLM = positionAtBreak.getLM();
    ...
    private int optimizeLineLength(final KnuthSequence effectiveList,
        ...
        while (effectiveListIterator.nextIndex() <= endElementIndex) {
            final KnuthElement tempEl = effectiveListIterator.next();
    ...
    (final BlockSequence blockList, final PageBreakingAlgorithm alg,
            final int availableBPD) {
        ...
        //
        final ListIterator<KnuthElement> sequenceIterator = blockList
                .listIterator();
        ...
        while (breakIterator.hasNext()) {
            ...
            final LinkedList<KnuthGlue> unconfirmedList = new LinkedList<KnuthGlue>();
            final LinkedList<KnuthGlue> adjustableLinesList = new LinkedList<KnuthGlue>();
            ...
            while (sequenceIterator.hasNext()
                ...
                if (thisElement.isGlue()) {
                    ...
                    final KnuthGlue thisGlue = (KnuthGlue) thisElement;
                    ...
                    if (!bBoxSeen) {
                        ...
                        while (!unconfirmedList.isEmpty()) {
                            ...
                            // blockSpaceList
                            final KnuthGlue blockSpace = unconfirmedList
                                    .removeFirst();
    ...
    private int adjustLineNumbers(final LinkedList<KnuthGlue> lineList,
            final int difference, final int total) {
        if (log.isDebugEnabled()) {
        ...
        final ListIterator<KnuthGlue> lineListIterator = lineList
                .listIterator();
        int adjustedDiff = 0;
        int partial = 0;
        while (lineListIterator.hasNext()) {
            final KnuthGlue line = lineListIterator.next();
