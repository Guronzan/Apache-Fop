         ...
         */
         ...
         while (posIter.hasNext()) {
             ...
             if (tbpNext.getLeafPos() != -1) {
                 areaInfo = (AreaInfo) this.areaInfos.get(tbpNext.getLeafPos());
     ...
     public List getNextKnuthElements(final LayoutContext context,
         ...
         while (this.nextStart < this.foText.length()) {
            ...
            case LineBreakStatus.INDIRECT_BREAK:
                ...
                break;
            default:
                TextLayoutManager.log.error("Unexpected breakAction: "
                        + breakAction);
             }
             if (log.isDebugEnabled()) {
                 log.debug("GK: {" + " index = " + this.nextStart + ", char = "
                        + CharUtilities.charToNCRef(ch) + ", level = " + level
                        + ", levelPrev = " + prevLevel + ", inWord = " + inWord
