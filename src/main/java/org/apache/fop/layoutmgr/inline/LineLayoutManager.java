...
BlockLevelLayoutManager {
    ...
    private static class LineBreakPosition extends LeafPosition {
        ...
        LineBreakPosition(
                ...
                final LayoutManager lm, final int index, final int startIndex,
                final int breakIndex, final int shrink, final int stretch,
                final int diff, final double ipdA, final double adjust,
                final int si, final int ei, final int lh, final int lw,
    ...
    private final class Update {
        ...
        private Update(final InlineLevelLayoutManager lm, final int index) {
            this.inlineLM = lm;
    ...
    private class LineBreakingAlgorithm extends BreakingAlgorithm {
        ...
        public LineBreakingAlgorithm(
                // CSOK: ParameterNumber
                final int pageAlign, final int textAlign,
        ...
        public void updateData2(final KnuthNode bestActiveNode,
            ...
            /*
             * startIndent += (textAlign == Constants.EN_CENTER) ? difference /
             * 2 : (textAlign == Constants.EN_END) ? difference : 0;
             */
            startIndent += bestActiveNode.line == 1 && this.indentFirstPart
            ...
            if (log.isWarnEnabled()) {
                ...
                // warning event here and instead let the BCLM handle that at a
                // later stage
        ...
        private LineBreakPosition makeLineBreakPosition(
                ...
                final double ratio, final int startIndent, final int endIndent) {
            ...
            if (isZeroHeightLine) {
                return new LineBreakPosition(this.thisLLM,
                        ...
                        availableStretch, difference, ratio, 0, startIndent,
                        endIndent, 0, LineLayoutManager.this.ipd, 0, 0, 0);
            } else {
                return new LineBreakPosition(this.thisLLM,
                        LineLayoutManager.this.knuthParagraphs.indexOf(par),
                        firstElementIndex, lastElementIndex, availableShrink,
                        availableStretch, difference, ratio, 0, startIndent,
                        endIndent, lineLead + lineFollow,
                        LineLayoutManager.this.ipd, spaceBefore, spaceAfter,
                        lineLead);
            }
        }
    ...
    public List getNextKnuthElements(final LayoutContext context,
        ...
        if (this.alignmentContext == null) {
            ...
            final FontTriplet[] fontkeys = this.fobj.getCommonFont()
                    .getFontState(fi);
    ...
    public List getNextKnuthElements(final LayoutContext context,
        ...
        final KnuthSequence paragraph = this.knuthParagraphs.get(parIndex);
        paragraph.subList(0, restartPosition.getIndex() + 1).clear();
        final Iterator<KnuthElement> iter = paragraph.iterator();
     ...
     * @param isLastPar
     *            flag indicating whether currPar is the last paragraph
    ...
    private LineLayoutPossibilities findOptimalBreakingPoints(
        ...
        final boolean canWrap = this.wrapOption != EN_NO_WRAP;
        final boolean canHyphenate = canWrap
        ...
        int breakingPoints = alg.findBreakingPoints(currPar, maxAdjustment,
                false, allowedBreaks);
        ...
        if (breakingPoints == 0 || alignment == EN_JUSTIFY) {
            ...
            breakingPoints = alg.findBreakingPoints(currPar, maxAdjustment,
                    false, allowedBreaks);
            if (breakingPoints == 0) {
    ...
    private List<ListElement> postProcessLineBreaks(final int alignment,
        ...
        for (int p = 0; p < this.knuthParagraphs.size(); p++) {
            ...
            if (!seq.isInlineSequence()) {
                final List<ListElement> targetList = new LinkedList<ListElement>();
                ...
                for (int i = 0; i < llPoss.getChosenLineCount(); ++i) {
                    ...
                    final List<LayoutManager> footnoteList = new LinkedList<LayoutManager>();
                    final ListIterator<KnuthElement> elementIterator = seq
                            .listIterator(startIndex);
                    while (elementIterator.nextIndex() <= endIndex) {
                        final KnuthElement element = elementIterator.next();
                        if (element instanceof KnuthInlineBox
    ...
    private void createElements(final List<ListElement> list,
         ...
         * lines
         */
        final List<KnuthElement> breaker = new LinkedList<KnuthElement>();
    ...
    private HyphContext getHyphenContext(final StringBuilder sbChars) {
        ...
        final Hyphenation hyph = Hyphenator.hyphenate(
                ...
                .getHyphenationTreeResolver(), getFObj().getUserAgent()
                .getFactory().getHyphPatNames(), sbChars.toString(),
    ...
    public boolean handleOverflow(final int milliPoints) {
        if (getParent() instanceof BlockLayoutManager) {
            return ((BlockLayoutManager) getParent())
