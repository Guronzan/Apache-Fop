...
/**
 * LayoutManager for a table-caption FO. The table caption contains blocks that
 * are placed beside the table.
 *
...
public class TableCaptionLayoutManager extends BlockStackingLayoutManager {
     ...
     *            table-caption FO
     */
    public TableCaptionLayoutManager(final TableCaption node) {
        super(node);
    }
     ...
     *
     * @param context
     *            the layout context for finding breaks
     ...
     * boolean over = false;
     *
     * while (!curLM.isFinished()) { if ((bp = curLM.getNextBreakPoss(childLC))
     * != null) { if (stackSize.opt + bp.getStackingSize().opt >
     * context.getStackLimit().max) { // reset to last break if (lastPos !=
     * null) { LayoutManager lm = lastPos.getLayoutManager();
     * lm.resetPosition(lastPos.getPosition()); if (lm != curLM) {
     * curLM.resetPosition(null); } } else { curLM.resetPosition(null); } over =
     * true; break; } stackSize.add(bp.getStackingSize()); lastPos = bp;
     * childBreaks.add(bp);
     *
     ...
     *
     * @param parentIter
     *            the position iterator of the breaks
     * @param layoutContext
