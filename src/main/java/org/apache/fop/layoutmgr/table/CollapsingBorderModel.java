...
/**
 * This class is a superclass for the two collapsing border models defined in
 * the XSL 1.0 specification.
 */
public abstract class CollapsingBorderModel {
     ...
     */
    public static CollapsingBorderModel getBorderModelFor(
            final int borderCollapse) {
     ...
     */
    public/* TODO */static int getOtherSide(final int side) {
        switch (side) {
     ...
     */
    protected boolean isVerticalRelation(final int side) {
        return side == CommonBorderPaddingBackground.BEFORE
                || side == CommonBorderPaddingBackground.AFTER;
    }

    private static int compareInt(final int value1, final int value2) {
        if (value1 < value2) {
            return -1;
     ...
     */
    private static int getStylePreferenceValue(final int style) {
        switch (style) {
     ...
     * @return a value &lt; 0 if style1 has less priority than style2, 0 if both
     *         are equal, a value &gt; 0 if style1 has more priority than style2
     */
    static int compareStyles(final int style1, final int style2) {
        final int value1 = getStylePreferenceValue(style1);
        final int value2 = getStylePreferenceValue(style2);
        return compareInt(value1, value2);
    }

    private static int getHolderPreferenceValue(final int id) {
        switch (id) {
     ...
     *
     * @param id1
     *            a FO id ({@link Constants#FO_TABLE},
     *            {@link Constants#FO_TABLE_BODY}, etc.)
     * @param id2
     *            another FO id
     * @return a value &lt; 0 if id1 has less priority than id2, 0 if both are
     *         equal, a value &gt; 0 if id1 has more priority than id2
     */
    static int compareFOs(final int id1, final int id2) {
        final int p1 = getHolderPreferenceValue(id1);
        final int p2 = getHolderPreferenceValue(id2);
        return compareInt(p1, p2);
    }

    /**
     * Returns the border which wins the border conflict resolution. In case the
     * two borders are equivalent (identical, or only the color is different),
     * null is returned.
     *
     * @param border1
     ...
     * @param discard
     *            true if the .conditionality component of the border width must
     *            be taken into account
     * @return the winning border, null if the two borders are equivalent
     */
    public abstract BorderSpecification determineWinner(
