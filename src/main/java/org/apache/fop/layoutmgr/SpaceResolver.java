...
public class SpaceResolver {
     ...
     */
    private SpaceResolver(final List<ListElement> first,
            final BreakElement breakPoss, final List<ListElement> second,
    ...
    private void generate(final ListIterator<KnuthElement> iter) {
        ...
        if (this.breakPoss != null) {
            ...
            // No break
            // TODO: We can't use a MinOptMax for glue2, because min <= opt <=
            // max is not always true - why?
        ...
        if (spaceAfterBreak.isNonZero()) {
            iter.add(new KnuthPenalty(0, KnuthElement.INFINITE, false, null,
                    true));
        ...
        while (iter.hasNext()) {
            ...
            if (el.isUnresolvedElement()) {
                ...
                if (breakPoss == null && unresolvedSecond.size() == 0 && !last) {
                    ...
                    // happen
                    final List<ListElement> swapList = unresolvedSecond;
