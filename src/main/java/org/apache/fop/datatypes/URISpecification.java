...
/* $Id: URISpecification.java 679326 2008-07-24 09:35:34Z vhennebert $ */
...
/**
 * This class contains method to deal with the <uri-specification> datatype from
 * XSL-FO.
 */
public class URISpecification {
    ...
    private static boolean isValidURIChar(final char ch) {
        return true;
    }
    ...
    private static boolean isReserved(final char ch) {
    ...
    private static boolean isUnreserved(final char ch) {
    ...
    private static void appendEscape(final StringBuilder sb, final byte b) {
        sb.append('%').append(HEX_DIGITS[b >> 4 & 0x0f])
                .append(HEX_DIGITS[b >> 0 & 0x0f]);
    }

    /**
     * Escapes any illegal URI character in a given URI, for example, it escapes
     * a space to "%20". Note: This method does not "parse" the URI and
     * therefore does not treat the individual components (user-info, path,
     * query etc.) individually.
     * 
     * @param uri
     *            the URI to inspect
    ...
    public static String escapeURI(String uri) {
        ...
        for (int i = 0, c = uri.length(); i < c; i++) {
            ...
            if (isReserved(ch) || isUnreserved(ch)) {
                ...
                try {
                    final byte[] utf8 = Character.toString(ch)
                            .getBytes("UTF-8");
                    for (final byte element : utf8) {
                        appendEscape(sb, element);
                    }
