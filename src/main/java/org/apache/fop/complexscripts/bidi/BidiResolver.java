...
public final class BidiResolver {
    ...
    public static void resolveInlineDirectionality(final PageSequence ps) {
        ...
        final List ranges = pruneEmptyRanges(ps
                .collectDelimitedTextRanges(new Stack()));
    ...
    private static void resolveInlineDirectionality(final List ranges) {
    ...
    private static int[] computeMinMaxLevel(final List runs, int[] mm) {
        if (mm == null) {
    ...
    private static List reorderRuns(List runs, final int level) {
        ...
        for (int i = 0, n = runs.size(); i < n; i++) {
            ...
            if (iri.getMinLevel() < level) {
                ...
                int e = s;
                while (e < n) {
                    final InlineRun ire = (InlineRun) runs.get(e);
    ...
    private static void reverseWords(final List runs, final boolean mirror) {
        for (final Iterator it = runs.iterator(); it.hasNext();) {
            final InlineRun ir = (InlineRun) it.next();
    ...
    private static List replicateSplitWords(final List runs) {
        // [TBD] for each run which inline word area appears multiple times in
        // runs, replicate that word
        return runs;
    }

    private static void replaceInlines(final LineArea la, final List runs) {
    ...
    private static void dumpRuns(final String header, final List runs) {
        log.debug(header);
        for (final Iterator it = runs.iterator(); it.hasNext();) {
            final InlineRun ir = (InlineRun) it.next();
