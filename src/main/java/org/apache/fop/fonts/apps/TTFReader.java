...
public class TTFReader extends AbstractFontReader {
     ...
     *            first, then the -er file. -nocs if complex script features are
     *            disabled
     */
     ...
     * @param useKerning
     *            true if should load kerning data
     * @param useAdvanced
     *            true if should load advanced typographic table data
    ...
    private void generateDOM4MultiByteExtras(final Element parent,
        ...
        mel.appendChild(el);
        for (final CMapSegment ce : ttf.getCMaps()) {
    ...
    private void generateDOM4Kerning(final Element parent, final TTFFile ttf,
        ...
        for (final Integer kpx1 : kerningKeys) {
            ...
            for (final Integer kpx2 : h2.keySet()) {
                if (isCid || kpx2.intValue() < 256) {
                    el2 = doc.createElement("pair");
                    el2.setAttribute("kpx2", kpx2.toString());
                    final Integer val = h2.get(kpx2);
