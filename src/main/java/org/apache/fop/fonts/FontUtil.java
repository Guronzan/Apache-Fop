...
public class FontUtil {
    ...
    /**
     * Parses an CSS2 (SVG and XSL-FO) font weight (normal, bold, 100-900) to an
     * integer. See http://www.w3.org/TR/REC-CSS2/fonts.html#propdef-font-weight
     ...
     */
    public static int parseCSS2FontWeight(final String text) {
        int weight = 400;
        try {
            ...
            if (text.equals("normal")) {
                ...
                throw new IllegalArgumentException(
                        "Illegal value for font weight: '" + text
    ...
    private static final String[] ITALIC_WORDS = { Font.STYLE_ITALIC,
            Font.STYLE_OBLIQUE, Font.STYLE_INCLINED };
     ...
     * weight
     */
    private static final String[] EXTRA_BOLD_WORDS = { "extrabold",
            "extra bold", "black", "heavy", "ultra", "super" };
     ...
     */
    public static String guessStyle(final String fontName) {
        if (fontName != null) {
            for (final String element : ITALIC_WORDS) {
                if (fontName.indexOf(element) != -1) {
     ...
     */
    public static int guessWeight(final String fontName) {
        // weight
        int weight = Font.WEIGHT_NORMAL;

        for (final String element : BOLD_WORDS) {
            if (fontName.indexOf(element) != -1) {
                weight = Font.WEIGHT_BOLD;
                break;
            }
        }
        for (final String element : MEDIUM_WORDS) {
            if (fontName.indexOf(element) != -1) {
        ...
        // semi/demi-light)
        for (final String element : DEMI_WORDS) {
            if (fontName.indexOf(element) != -1) {
                weight = Font.WEIGHT_BOLD - 100; // 600
                break;
            }
        }
        for (final String element : EXTRA_BOLD_WORDS) {
            if (fontName.indexOf(element) != -1) {
                weight = Font.WEIGHT_EXTRA_BOLD;
                break;
            }
        }
        for (final String element : LIGHT_WORDS) {
            if (fontName.indexOf(element) != -1) {
