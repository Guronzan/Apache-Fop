...
public class PSRenderingUtil implements PSConfigurationConstants {
    ...
    /**
     * Whether or not PostScript Document Structuring Conventions (DSC)
     * compliant output are enforced.
     */
    ...
    /**
     * Determines whether the PS file is generated in two passes to minimize
     * file size
     */
    private boolean optimizeResources = false;

    PSRenderingUtil(final FOUserAgent userAgent) {
    ...
    private void initialize() {
        Object obj;
        obj = this.userAgent.getRendererOptions().get(AUTO_ROTATE_LANDSCAPE);
        if (obj != null) {
            setAutoRotateLandscape(booleanValueOf(obj));
        }
        obj = this.userAgent.getRendererOptions().get(LANGUAGE_LEVEL);
        if (obj != null) {
            setLanguageLevel(intValueOf(obj));
        }
        obj = this.userAgent.getRendererOptions().get(OPTIMIZE_RESOURCES);
        if (obj != null) {
            setOptimizeResources(booleanValueOf(obj));
        }
    }

    private boolean booleanValueOf(final Object obj) {
    ...
    private int intValueOf(final Object obj) {
        if (obj instanceof Integer) {
            ...
            throw new IllegalArgumentException(
                    "Integer or String with a number expected.");
        }
    }
     ...
     *            the PS generator
     * @param setupCodeList
     *            a List of PSSetupCode instances
     * @param type
     *            the type of code section
     * @throws IOException
    ...
    /**
     * Formats and writes a Collection of PSExtensionAttachment instances to the
     * output stream. The instances are removed from the collection when they
     * have been written.
     *
     * @param gen
     *            the PS generator
     ...
     *             if an I/O error occurs.
     */
    public static void writeEnclosedExtensionAttachment(final PSGenerator gen,
            final PSExtensionAttachment attachment) throws IOException {
        if (attachment instanceof PSCommentBefore) {
            ...
            if (attachment instanceof PSSetupCode) {
                final PSSetupCode setupCodeAttach = (PSSetupCode) attachment;
                final String name = setupCodeAttach.getName();
                if (name != null) {
                    info += ": (" + name + ")";
                }
            }
     ...
     */
    public void setSafeSetPageDevice(final boolean value) {
     ...
     */
    public void setDSCComplianceEnabled(final boolean value) {
    ...
    public void setLanguageLevel(final int level) {
        if (level == 2 || level == 3) {
            ...
            throw new IllegalArgumentException(
                    "Only language levels 2 or 3 are allowed/supported");
        }
    }
