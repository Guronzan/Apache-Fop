...
/**
 * This class is used when two-pass production is used to generate the
 * PostScript file (setting "optimize-resources"). It uses the DSC parser from
 * XML Graphics Commons to go over the temporary file generated by the
...
public class ResourceHandler implements DSCParserConstants, PSSupportedFlavors {
    ...
    private final PSEventProducer eventProducer;

    private final ResourceTracker resTracker;

    // key: URI, values PSImageFormResource
    private final Map globalFormResources = new HashMap<>();
    // key: PSResource, values PSImageFormResource
    private final Map inlineFormResources = new HashMap<>();
     ...
     * @param resTracker
     *            the resource tracker to use
     * @param formResources
     *            Contains all forms used by this document (maintained by
     *            PSRenderer)
     */
    public ResourceHandler(final FOUserAgent userAgent,
            final PSEventProducer eventProducer, final FontInfo fontInfo,
            final ResourceTracker resTracker, final Map formResources) {
    ...
    /**
     * This method splits up the form resources map into two. One for global
     * forms which have been referenced more than once, and one for inline forms
     * which have only been used once. The latter is to conserve memory in the
    ...
    /**
     * Rewrites the temporary PostScript file generated by PSRenderer adding all
     * needed resources (fonts and images).
     *
     * @param in
     *            the InputStream for the temporary PostScript file
     * @param out
     *            the OutputStream to write the finished file to
     * @param pageCount
     *            the number of pages (given here because PSRenderer writes an
     *            "(atend)")
     * @param documentBoundingBox
     *            the document's bounding box (given here because PSRenderer
     *            writes an "(atend)")
     * @throws DSCException
     *             If there's an error in the DSC structure of the PS file
     * @throws IOException
    ...
    public void process(final InputStream in, final OutputStream out,
        ...
        parser.setFilter(new DSCFilter() {
            private final Set filtered = new HashSet();
            {
                ...
                this.filtered.add(DSCConstants.BBOX);
            ...
            public boolean accept(final DSCEvent event) {
                if (event.isDSCComment()) {
                    ...
                    return !this.filtered.contains(event.asDSCComment()
                            .getName());
                } else {
                    return true;
                }
        ...
        while (true) {
            ...
            if (DSCTools.headerCommentsEndHere(event)) {
                // Set number of pages
                final DSCCommentPages pages = new DSCCommentPages(pageCount);
                pages.generate(gen);
                ...
                // Supplied Resources
                final DSCCommentDocumentSuppliedResources supplied = new DSCCommentDocumentSuppliedResources(
                        this.resTracker.getDocumentSuppliedResources());
                supplied.generate(gen);

                // Needed Resources
                final DSCCommentDocumentNeededResources needed = new DSCCommentDocumentNeededResources(
                        this.resTracker.getDocumentNeededResources());
                needed.generate(gen);
            ...
            if (event.isDSCComment()) {
                ...
                if (DSCConstants.LANGUAGE_LEVEL.equals(comment.getName())) {
                    final DSCCommentLanguageLevel level = (DSCCommentLanguageLevel) comment;
        ...
        final PostScriptComment fontSetupPlaceholder = parser.nextPSComment(
                "FOPFontSetup", gen);
        if (fontSetupPlaceholder == null) {
            throw new DSCException(
                    "Didn't find %FOPFontSetup comment in stream");
        }
        PSFontUtils.writeFontDict(gen, this.fontInfo,
                this.fontInfo.getUsedFonts(), this.eventProducer);
        generateForms(this.globalFormResources, gen);
    ...
    private void generateFormForImage(final PSGenerator gen,
        ...
        try {
            ...
            final PSRenderingContext formContext = new PSRenderingContext(
                    this.userAgent, gen, this.fontInfo, true);
            ...
            final ImageHandler basicHandler = imageHandlerRegistry.getHandler(
                    formContext, img);
            ...
            final PSImageHandler handler = (PSImageHandler) basicHandler;
    ...
    private class IncludeResourceListener implements DSCListener {
        ...
        public IncludeResourceListener(final PSGenerator gen) {
            this.gen = gen;
        }
        ...
        public void processEvent(final DSCEvent event, final DSCParser parser)
            ...
            if (event.isDSCComment()
                    && event instanceof DSCCommentIncludeResource) {
                final DSCCommentIncludeResource include = (DSCCommentIncludeResource) event;
                final PSResource res = include.getResource();
                if (res.getType().equals(PSResource.TYPE_FORM)) {
                    if (ResourceHandler.this.inlineFormResources
                            .containsValue(res)) {
                        final PSImageFormResource form = (PSImageFormResource) ResourceHandler.this.inlineFormResources
                                .get(res);
                        // Create an inline form
                        // Wrap in save/restore pair to release memory
                        this.gen.writeln("save");
                        generateFormForImage(this.gen, form);
                        boolean execformFound = false;
                        final DSCEvent next = parser.nextEvent();
                        if (next.isLine()) {
                            final PostScriptLine line = next.asLine();
