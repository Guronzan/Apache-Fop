...
/**
 * Generic XML handler for SVG. Uses Apache Batik for SVG processing and simply
 * paints to a Graphics2DAdapter and thus ultimatively to Graphics2D interface
 * that is presented.
 * <p>
 * To use this class, subclass it and implement the missing methods
 * (supportsRenderer, for example).
 */
...
RendererContextConstants {
    ...
    protected Graphics2DImagePainter createGraphics2DImagePainter(
            final GraphicsNode root, final BridgeContext ctx,
     ...
     */
    protected GraphicsNode buildGraphicsNode(final FOUserAgent userAgent,
            final BridgeContext ctx, final Document doc) {
        final GVTBuilder builder = new GVTBuilder();
        final GraphicsNode root;
        try {
            root = builder.build(ctx, doc);
        } catch (final Exception e) {
            final EventBroadcaster eventBroadcaster = userAgent
                    .getEventBroadcaster();
     ...
     */
    protected Dimension getImageSize(final RendererContextWrapper wrappedContext) {
     ...
     */
    protected String getDocumentURI(final Document doc) {
        String docURI = null;
        if (doc instanceof AbstractDocument) {
            final AbstractDocument level3Doc = (AbstractDocument) doc;
            docURI = level3Doc.getNamespaceURI();
        }
        return docURI;
    }

    /**
     * Override this method to update the renderer context if it needs special
     * settings for certain conditions.
     *
     * @param context
     *            the renderer context
     */
