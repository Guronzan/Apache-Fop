...
public class XMLHandlerRegistry {
    ...
    public XMLHandler getXMLHandler(final Renderer renderer, final String ns) {
        ...
        if (handler == null) {
            lst = (List) this.handlers.get(XMLHandler.HANDLE_ALL);
            handler = getXMLHandler(renderer, lst);
        }
        return handler;
    }
    ...
    private void discoverXMLHandlers() {
        ...
        final Iterator providers = Service.providers(XMLHandler.class);
        if (providers != null) {
            while (providers.hasNext()) {
                final XMLHandler handler = (XMLHandler) providers.next();
                try {
