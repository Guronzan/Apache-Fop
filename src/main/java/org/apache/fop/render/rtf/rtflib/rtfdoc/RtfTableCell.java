...
public class RtfTableCell extends RtfContainer implements
        IRtfParagraphContainer, IRtfListContainer, IRtfTableContainer,
    ...
    /**
     * cell merging: this cell is part of (but not the start of) a range of
     * merged cells
     */
    public static final int MERGE_WITH_PREVIOUS = 2;

    /** Create an RTF element as a child of given container */
    RtfTableCell(final RtfTableRow parent, final Writer w, final int cellWidth,
            final int idNum) throws IOException {
        super(parent, w);
        this.id = idNum;
    ...
    /** Create an RTF element as a child of given container */
    RtfTableCell(final RtfTableRow parent, final Writer w, final int cellWidth,
            final RtfAttributes attrs, final int idNum) throws IOException {
     ...
     */
    @Override
    public RtfList newList(final RtfAttributes attrib) throws IOException {
        closeAll();
    ...
    public RtfTable newTable(final RtfAttributes attrs,
        ...
        closeAll();
        this.table = new RtfTable(this, this.writer, attrs, tc); // Added tc
                                                                 // Boris
                                                                 // Poudérous
                                                                 // 07/22/2002
        return this.table;
     ...
     */
    int writeCellDef(final int offset) throws IOException {
        /*
         * Don't write \clmgf or \clmrg. Instead add the widths of all spanned
         * columns and create a single wider cell, because \clmgf and \clmrg
         * won't work in last row of a table (Word2000 seems to do the same).
         * Cause of this, dont't write horizontally merged cells. They just
         * exist as placeholders in TableContext class, and are never written to
         * RTF file.
         */
        // horizontal cell merge codes
        if (this.hMerge == MERGE_WITH_PREVIOUS) {
            return offset;
        }
        ...
        if (this.vMerge == MERGE_START) {
            writeControlWord("clvmgf");
        } else if (this.vMerge == MERGE_WITH_PREVIOUS) {
        ...
        if (this.attrib.getValue("number-columns-spanned") != null) {
            ...
            final int nbMergedCells = ((Integer) this.attrib
                    .getValue("number-columns-spanned")).intValue();

            final RtfTable tab = getRow().getTable();

            // Get the context of the current table in order to get the width of
            // each column
            final ITableColumnsInfo tableColumnsInfo = tab
        ...
        writeAttributes(this.attrib, ITableAttributes.CELL_VERT_ALIGN);
    ...
    /**
     * Called before writeRtfContent; overriden to avoid writing any it's a
     * merged cell.
     * 
     * @throws IOException
     *             for I/O problems
    ...
    protected void writeRtfSuffix() throws IOException {
        ...
        if (getRow().getTable().isNestedTable()) {
            ...
            // word97 hangs if cell does not contain at least one "par" control
            // word
            ...
            if (this.setCenter) {
                ...
                if (getChildren().size() > 0) {
                    lastChild = (RtfElement) getChildren().get(
                            getChildren().size() - 1);
                }

                if (lastChild != null && lastChild instanceof RtfTextrun) {
                    // Don't write \ql in order to allow for example a right
                    // aligned paragraph
     ...
     */
    public void setVMerge(final int mergeStatus) {
     ...
     *            mergeStatus to set
     */
    public void setHMerge(final int mergeStatus) {
    ...
    /**
     * Overridden so that nested tables cause extra rows to be added after the
     * row that contains this cell disabled for V0.3 - nested table support is
     * not done yet
     * 
     * @throws IOException
     *             for I/O problems
     */
    /*
     * protected void writeRtfContent() throws IOException { int extraRowIndex =
     * 0; RtfTableCell extraCell = null;
     * 
     * for (Iterator it = getChildren().iterator(); it.hasNext();) { final
     * RtfElement e = (RtfElement)it.next(); if (e instanceof RtfTable) { //
     * nested table - render its cells in supplementary rows after current row,
     * // and put the remaining content of this cell in a new cell after nested
     * table // Line added by Boris Poudérous
     ...
     * extraRowIndex, widthOffset); // Boris Poudérous added the passing of the
     * current cell // attributes to the new cells (in order not to have cell
     * without // border for example) extraCell =
     * parentRow.getExtraRowSet().createExtraCell(extraRowIndex, widthOffset,
     * this.getCellWidth(), attrib); extraRowIndex++;
     * 
     * } else if (extraCell != null) { // we are after a nested table, add
     * elements to the extra cell created for them extraCell.addChild(e);
     * 
     * } else { // before a nested table, normal rendering e.writeRtf(); } } }
     */

    /**
     * A table cell always contains "useful" content, as it is here to take some
     * space in a row. Use containsText() to find out if there is really some
     * useful content in the cell. TODO: containsText could use the original
     ...
     */
    boolean paragraphNeedsPar(final RtfParagraph p) {
        // true if there is at least one non-empty paragraph after p in our
        // children
    ...
    public RtfTextrun getTextrun() throws IOException {
        final RtfAttributes attrs = new RtfAttributes();
