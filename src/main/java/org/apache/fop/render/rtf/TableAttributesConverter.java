...
public final class TableAttributesConverter {
    ...
    static RtfAttributes convertCellAttributes(final TableCell fobj) {
        ...
        final CommonBorderPaddingBackground border = fobj
        ...
        if (color == null) {
            ...
            if (color == null
                ...
                final Table table = (Table) fobj.getParent().getParent()
                        .getParent();
        ...
        BorderAttributesConverter.makeBorder(border,
                CommonBorderPaddingBackground.BEFORE, attrib,
                ITableAttributes.CELL_BORDER_TOP);
        BorderAttributesConverter.makeBorder(border,
                CommonBorderPaddingBackground.AFTER, attrib,
                ITableAttributes.CELL_BORDER_BOTTOM);
        BorderAttributesConverter.makeBorder(border,
                CommonBorderPaddingBackground.START, attrib,
                ITableAttributes.CELL_BORDER_LEFT);
        BorderAttributesConverter.makeBorder(border,
                CommonBorderPaddingBackground.END, attrib,
                ITableAttributes.CELL_BORDER_RIGHT);

        int padding;
        final boolean reproduceMSWordBug = true;
        ...
        final int n = fobj.getNumberColumnsSpanned();
        // Column spanning :
        if (n > 1) {
    ...
    static RtfAttributes convertRowAttributes(final TableRow fobj,
         ...
         * value will be taken out by RtfTableRow. This is because you can't say
         * BORDER_TOP and BORDER_HORIZONTAL if the cell lies at the top of the
         ...
         * convertAttributetoRtf(ep.getEnum())); isBorderPresent = true; } ep =
         * (EnumProperty)fobj.getProperty(Constants.PR_BORDER_BOTTOM_STYLE); if
         * (ep != null && ep.getEnum() != Constants.EN_NONE) {
         * attrib.set(ITableAttributes.ROW_BORDER_BOTTOM, "\\" +
         * convertAttributetoRtf(ep.getEnum()));
         * attrib.set(ITableAttributes.ROW_BORDER_HORIZONTAL, "\\" +
         * convertAttributetoRtf(ep.getEnum())); isBorderPresent = true; } ep =
         * (EnumProperty)fobj.getProperty(Constants.PR_BORDER_LEFT_STYLE); if
         * (ep != null && ep.getEnum() != Constants.EN_NONE) {
         * attrib.set(ITableAttributes.ROW_BORDER_LEFT, "\\" +
         * convertAttributetoRtf(ep.getEnum()));
         * attrib.set(ITableAttributes.ROW_BORDER_VERTICAL, "\\" +
         * convertAttributetoRtf(ep.getEnum())); isBorderPresent = true; } ep =
         * (EnumProperty)fobj.getProperty(Constants.PR_BORDER_RIGHT_STYLE); if
         * (ep != null && ep.getEnum() != Constants.EN_NONE) {
