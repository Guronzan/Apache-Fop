...
public class RtfTableRow extends RtfContainer implements ITableAttributes {
    ...
    RtfTableRow(final RtfTable parent, final Writer w, final int idNum)
            throws IOException {
        super(parent, w);
        this.id = idNum;
    }
    ...
    /** Create an RTF element as a child of given container */
    RtfTableRow(final RtfTable parent, final Writer w,
     ...
     *             for I/O problems
     */
    public RtfTableCell newTableCell(final int cellWidth) throws IOException {
        this.highestCell++;
        this.cell = new RtfTableCell(this, this.writer, cellWidth,
                this.highestCell);
        return this.cell;
    }
     ...
     *             for I/O problems
     */
    public RtfTableCell newTableCell(final int cellWidth,
            final RtfAttributes attrs) throws IOException {
        this.highestCell++;
        this.cell = new RtfTableCell(this, this.writer, cellWidth, attrs,
                this.highestCell);
        return this.cell;
    }

    /**
     * Added by Boris POUDEROUS on 07/02/2002 in order to add an empty cell that
     * is merged with the cell above. This cell is placed before or after the
     * nested table.
     * 
     * @param attrs
     *            attributes of new cell
     ...
     *             for I/O problems
     */
    public RtfTableCell newTableCellMergedVertically(final int cellWidth,
            final RtfAttributes attrs) throws IOException {
        this.highestCell++;
        this.cell = new RtfTableCell(this, this.writer, cellWidth, attrs,
                this.highestCell);
        this.cell.setVMerge(RtfTableCell.MERGE_WITH_PREVIOUS);
        return this.cell;
    }

    /**
     * Added by Boris POUDEROUS on 07/02/2002 in order to add an empty cell that
     * is merged with the previous cell.
     * 
     * @param attrs
     *            attributes of new cell
     * @param cellWidth
     ...
     *             for I/O problems
     */
    public RtfTableCell newTableCellMergedHorizontally(final int cellWidth,
            final RtfAttributes attrs) throws IOException {
        this.highestCell++;
        ...
        this.cell = new RtfTableCell(this, this.writer, cellWidth, wAttributes,
                this.highestCell);
        this.cell.setHMerge(RtfTableCell.MERGE_WITH_PREVIOUS);
        return this.cell;
    }
    ...
    protected void writeRtfContent() throws IOException {
        ...
        // now children can write themselves, we have the correct RTF prefix
        // code
    ...
    public void writeRowAndCellsDefintions() throws IOException {
        ...
        if (this.attrib != null
                && this.attrib.isSet(ITableAttributes.ROW_KEEP_TOGETHER)) {
            writeControlWord(ROW_KEEP_TOGETHER);
        }

        writePaddingAttributes();

        final RtfTable parentTable = (RtfTable) this.parent;
        adjustBorderProperties(parentTable);

        writeAttributes(this.attrib,
                new String[] { ITableAttributes.ATTR_HEADER });
        ...
        if (this.attrib.isSet(ITableAttributes.ROW_HEIGHT)) {
            writeOneAttribute(ITableAttributes.ROW_HEIGHT,
                    this.attrib.getValue(ITableAttributes.ROW_HEIGHT));
        }

        // write X positions of our cells
        int xPos = 0;

        final Object leftIndent = this.attrib
                .getValue(ITableAttributes.ATTR_ROW_LEFT_INDENT);
        if (leftIndent != null) {
            xPos = ((Integer) leftIndent).intValue();
        }

        final RtfAttributes tableBorderAttributes = getTable()
                .getBorderAttributes();
        ...
        for (final Iterator it = getChildren().iterator(); it.hasNext();) {
            ...
            if (e instanceof RtfTableCell) {
                ...
                final RtfTableCell rtfcell = (RtfTableCell) e;

                // Adjust the cell's display attributes so the table's/row's
                // borders
                // are drawn properly.
                ...
                if (tableBorderAttributes != null) {
                    ...
                    if (index == 0) {
                        final String border = ITableAttributes.CELL_BORDER_LEFT;
                        if (!rtfcell.getRtfAttributes().isSet(border)) {
                            rtfcell.getRtfAttributes().set(
                                    border,
                                    (RtfAttributes) tableBorderAttributes
                                            .getValue(border));
                        }
                    }

                    if (index == getChildCount() - 1) {
                        final String border = ITableAttributes.CELL_BORDER_RIGHT;
                        if (!rtfcell.getRtfAttributes().isSet(border)) {
                            rtfcell.getRtfAttributes().set(
                                    border,
                                    (RtfAttributes) tableBorderAttributes
                    ...
                    if (isFirstRow()) {
                        final String border = ITableAttributes.CELL_BORDER_TOP;
                        if (!rtfcell.getRtfAttributes().isSet(border)) {
                            rtfcell.getRtfAttributes().set(
                                    border,
                                    (RtfAttributes) tableBorderAttributes
                    ...
                    if (parentTable != null
                            && parentTable.isHighestRow(this.id)) {
                        final String border = ITableAttributes.CELL_BORDER_BOTTOM;
                        if (!rtfcell.getRtfAttributes().isSet(border)) {
                            rtfcell.getRtfAttributes().set(
                                    border,
                                    (RtfAttributes) tableBorderAttributes
                ...
                if (index == 0) {
                    if (!rtfcell.getRtfAttributes().isSet(
                            ITableAttributes.CELL_BORDER_LEFT)) {
                        rtfcell.getRtfAttributes()
                                .set(ITableAttributes.CELL_BORDER_LEFT,
                                        (String) this.attrib
                                                .getValue(ITableAttributes.ROW_BORDER_LEFT));
                    }
                }
                ...
                if (index == getChildCount() - 1) {
                    if (!rtfcell.getRtfAttributes().isSet(
                        ...
                        rtfcell.getRtfAttributes()
                                .set(ITableAttributes.CELL_BORDER_RIGHT,
                                        (String) this.attrib
                                                .getValue(ITableAttributes.ROW_BORDER_RIGHT));
                    }
                }
                ...
                if (isFirstRow()) {
                    if (!rtfcell.getRtfAttributes().isSet(
                        ...
                        rtfcell.getRtfAttributes()
                                .set(ITableAttributes.CELL_BORDER_TOP,
                                        (String) this.attrib
                                                .getValue(ITableAttributes.ROW_BORDER_TOP));
                    }
                }
                ...
                if (parentTable != null && parentTable.isHighestRow(this.id)) {
                    if (!rtfcell.getRtfAttributes().isSet(
                        ...
                        rtfcell.getRtfAttributes()
                                .set(ITableAttributes.CELL_BORDER_BOTTOM,
                                        (String) this.attrib
                                                .getValue(ITableAttributes.ROW_BORDER_BOTTOM));
                    }
                }
    ...
    private void writePaddingAttributes() throws IOException {
        ...
        if (this.attrib != null && !this.attrib.isSet(ATTR_RTF_15_TRGAPH)) {
            int gaph = -1;
            try {
                // set (RTF 1.5) gaph to the average of the (RTF 1.6) left and
                // right padding values
                final Integer leftPadStr = (Integer) this.attrib
                        .getValue(ATTR_ROW_PADDING_LEFT);
                if (leftPadStr != null) {
                    gaph = leftPadStr.intValue();
                }
                final Integer rightPadStr = (Integer) this.attrib
                        .getValue(ATTR_ROW_PADDING_RIGHT);
                if (rightPadStr != null) {
            ...
            if (gaph >= 0) {
                this.attrib.set(ATTR_RTF_15_TRGAPH, gaph);
            }
        }
     ...
     */
    public boolean isHighestCell(final int cellId) {
        return this.highestCell == cellId ? true : false;
    }
