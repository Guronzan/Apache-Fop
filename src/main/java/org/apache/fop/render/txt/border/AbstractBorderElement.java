...
public abstract class AbstractBorderElement implements Constants {
    ...
    /**
     * Constant for a line segment, directing from a center of symbol up the the
     * symbol boundary.
     */
    public static final int UP = 0;

    /**
     * Constant for a line segment, directing from a center of symbol right the
     * the symbol boundary.
     */
    public static final int RIGHT = 1;

    /**
     * Constant for a line segment, directing from a center of symbol down the
     * the symbol boundary.
     */
    public static final int DOWN = 2;

    /**
     * Constant for a line segment, directing from a center of symbol left the
     * the symbol boundary.
     */
    ...
    /**
     * Initializes a newly created <code>AbstractBorderElement</code> object so
     * that it represents an empty border element.
     */
     ...
     *
     * @param side
     *            integer, representing side
     * @return value of side's element
     */
    public int getData(final int side) {
     ...
     *            integer, representing side
     * @param value
     *            a new value for <code>data[side]</code>
     */
    public void setData(final int side, final int value) {
     ...
     *
     * @param state
     *            instance of TXTState
     */
    public void transformElement(final TXTState state) {
        // here we'll get CTM^-1 without shift
        final double[] da = state.getResultCTM().toArray();
        final CTM ctm = new CTM(da[0], -da[1], -da[2], da[3], 0, 0);

        final Point[] pa = new Point[4];
        pa[0] = new Point(0, this.data[UP]);
        pa[1] = new Point(this.data[RIGHT], 0);
        pa[2] = new Point(0, -this.data[DOWN]);
        ...
        for (int i = 0; i < 4; ++i) {
            ...
            if (p.x == 0 && p.y > 0) {
                ...
                this.data[DOWN] = length;
            } else if (p.x > 0 && p.y == 0) {
                this.data[RIGHT] = length;
            } else if (p.x < 0 && p.y == 0) {
                this.data[LEFT] = length;
            }
        }
    }
     ...
     */
    public abstract AbstractBorderElement merge(final AbstractBorderElement e);
