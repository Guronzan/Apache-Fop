...
public class CommandLineOptions {
    ...
    public boolean parse(final String[] args) throws FOPException, IOException {
        ...
        if (MimeConstants.MIME_FOP_AWT_PREVIEW.equals(this.outputmode)) {
            ...
            final AWTRenderer renderer = new AWTRenderer(this.foUserAgent,
                    this.inputHandler, true, true);
            ...
            final Renderer targetRenderer = this.foUserAgent
                    .getRendererFactory().createRenderer(this.foUserAgent,
                            this.mimicRenderer);
            final XMLRenderer xmlRenderer = new XMLRenderer(this.foUserAgent);
    ...
    private int parseCacheOption(final String[] args, final int i)
            throws FOPException {
        if (i + 1 == args.length || isOption(args[i + 1])) {
            ...
            this.factory.getFontManager().setCacheFile(new File(args[i + 1]));
            return 1;
        }
    }
    ...
    private void dumpConfiguration() {
        ...
        if (this.outputmode == null) {
            ...
            CommandLineOptions.log.info("awt on screen");
            if (this.outfile != null) {
                CommandLineOptions.log.error("awt mode, but outfile is set:");
                CommandLineOptions.log.error("out file: "
                        + this.outfile.toString());
            }
        } else if (MimeConstants.MIME_FOP_PRINT.equals(this.outputmode)) {
            CommandLineOptions.log.info("print directly");
            if (this.outfile != null) {
                CommandLineOptions.log.error("print mode, but outfile is set:");
                CommandLineOptions.log.error("out file: "
                        + this.outfile.toString());
            }
        } else if (MimeConstants.MIME_FOP_AREA_TREE.equals(this.outputmode)) {
            CommandLineOptions.log.info("area tree");
            if (this.mimicRenderer != null) {
                CommandLineOptions.log.info("mimic renderer: "
                        + this.mimicRenderer);
            }
            if (isOutputToStdOut()) {
                CommandLineOptions.log.info("output file: to stdout");
            } else {
                CommandLineOptions.log.info("output file: "
                        + this.outfile.toString());
            }
        } else if (MimeConstants.MIME_FOP_IF.equals(this.outputmode)) {
            CommandLineOptions.log.info("intermediate format");
            CommandLineOptions.log.info("output file: "
                    + this.outfile.toString());
        } else {
            CommandLineOptions.log.info(this.outputmode);
            if (isOutputToStdOut()) {
                CommandLineOptions.log.info("output file: to stdout");
            } else {
                CommandLineOptions.log.info("output file: "
        ...
        CommandLineOptions.log.info("OPTIONS");

        if (this.userConfigFile != null) {
            CommandLineOptions.log.info("user configuration file: "
    ...
    private void flushCache() throws FOPException {
        final FontManager fontManager = this.factory.getFontManager();
        final File cacheFile = fontManager.getCacheFile();
