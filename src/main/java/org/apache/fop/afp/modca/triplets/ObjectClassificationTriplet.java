...
/**
 * The Object Classification is used to classify and identify object data. The
 * object data may or may not be defined by an IBM presentation architecture
 */
public class ObjectClassificationTriplet extends AbstractTriplet {
    ...
    /**
     * This is not a presentation object, the scope of this object is not
     * defined
     */
    public static final byte CLASS_EXECUTABLE_PROGRAM = 0x20;

    /**
     * Setup information file, document level. This is not a presentation
     * object,
     */
    public static final byte CLASS_SETUP_FILE = 0x30;

    /**
     * This is a resource used by a presentation object that may itself be a
     * resource. The scope of the resource is the object that uses the resource.
     */
    public static final byte CLASS_SECONDARY_RESOURCE = 0x40;

    /**
     * Data object font. This is a non-FOCA font resource used to present text
     * in a data object. The scope of the resource is the object that uses the
     * resource.
     */
     ...
     *            field
     */
    public ObjectClassificationTriplet(final byte objectClass,
        ...
        this(objectClass, objectType, dataInContainer, containerHasOEG,
                dataInOCD, null, null);
    }
     ...
     * @param compName
     *            the name of the company or organization that owns the object
     *            definition
     */
    public ObjectClassificationTriplet(final byte objectClass,
            final ObjectType objectType, final boolean dataInContainer,
            final boolean containerHasOEG, final boolean dataInOCD,
            final String objLev, final String compName) {
        super(OBJECT_CLASSIFICATION);

        this.objectClass = objectClass;
        if (objectType == null) {
            throw new IllegalArgumentException(
                    "MO:DCA Registry object type is null");
        }
     ...
     *
     * @param dataInContainer
     *            true if the object data in carried in the object container
     * @param containerHasOEG
     *            true if the object container has an object environment group
     * @param dataInOCD
     *            true if the object container data carries the object data
     *
     * @return the byte value of this structure
     */
    public byte[] getStructureFlagsAsBytes(final boolean dataInContainer,
            final boolean containerHasOEG, final boolean dataInOCD) {
        final byte[] strucFlgs = new byte[2];
    ...
    public void writeToStream(final OutputStream os) throws IOException {
        ...
        data[3] = this.objectClass; // ObjClass
        ...
        final byte[] structureFlagsBytes = getStructureFlagsAsBytes(
                this.dataInContainer, this.containerHasOEG, this.dataInOCD);
        data[6] = structureFlagsBytes[0];
        data[7] = structureFlagsBytes[1];

        final byte[] objectIdBytes = this.objectType.getOID();
        ...
        objectTypeNameBytes = StringUtils.rpad(this.objectType.getName(), ' ',
                OBJECT_TYPE_NAME_LEN).getBytes(AFPConstants.EBCIDIC_ENCODING);
        System.arraycopy(objectTypeNameBytes, 0, data, 24,
                objectTypeNameBytes.length);

        // ObjLev - release level or version number of object type (56-63)
        byte[] objectLevelBytes;
        objectLevelBytes = StringUtils.rpad(this.objectLevel, ' ',
                OBJECT_LEVEL_LEN).getBytes(AFPConstants.EBCIDIC_ENCODING);
        System.arraycopy(objectLevelBytes, 0, data, 56, objectLevelBytes.length);

        // CompName - name of company or organization that owns object
        // definition (64-95)
        byte[] companyNameBytes;
        companyNameBytes = StringUtils.rpad(this.companyName, ' ',
                COMPANY_NAME_LEN).getBytes(AFPConstants.EBCIDIC_ENCODING);
