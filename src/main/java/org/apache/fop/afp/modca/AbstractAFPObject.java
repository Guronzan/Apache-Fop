 ...
 * responsible for building and generating the binary datastream in an AFP
 * format.
 */
@Slf4j
public abstract class AbstractAFPObject implements Streamable {
    ...
    protected static final byte[] SF_HEADER = new byte[] { 0x5A, // Structured
        // field
        // identifier
        0x00, // Length byte 1
        0x10, // Length byte 2
     ...
     *
     * @param data
     *            the structured field data byte array
     * @param type
     *            the type code
     * @param category
     *            the category code
     ...
     *            the category code
     */
    protected static void copySF(final byte[] data, final byte clazz,
            final byte type, final byte category) {
    ...
    /**
     * Reads data chunks from an InputStream and then formats them with a
     * structured header to a given OutputStream
     *
     * @param dataHeader
     *            the header data
     * @param lengthOffset
     *            offset of length field in data chunk
     ...
     */
    protected static void copyChunks(final byte[] dataHeader,
            final int lengthOffset, final int maxChunkLength,
        ...
        final byte[] data = new byte[maxChunkLength];
        int numBytesRead = 0;
        while ((numBytesRead = inputStream.read(data, 0, maxChunkLength)) > 0) {
            final byte[] len = BinaryUtils.convert(headerLen + numBytesRead, 2);
     ...
     *             thrown if an I/O exception of some sort has occurred.
     */
    protected static void writeChunksToStream(final byte[] data,
            final byte[] dataHeader, final int lengthOffset,
            final int maxChunkLength, final OutputStream os) throws IOException {
        final int dataLength = data.length;
        final int numFullChunks = dataLength / maxChunkLength;
        final int lastChunkLength = dataLength % maxChunkLength;
     ...
     */
    protected String truncate(String str, final int maxLength) {
