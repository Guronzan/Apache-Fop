...
@Slf4j
 public abstract class AbstractAFPObject implements Streamable {
    ...
    /** the structured field class id */
     ...
     private static final byte[] SF_HEADER = new byte[] { 0x5A, // Structured
                                                               // field
                                                               // identifier
            0x00, // Length byte 1
            0x10, // Length byte 2
     ...
     * @param <S>
     *            Streamable view of an AFPObject
     * @param os
     *            The stream to write to
     * @throws java.io.IOException
     *             an I/O exception of some sort has occurred.
      */
     protected <S extends Streamable> void writeObjects(
            final Collection<S> objects, final OutputStream os)
             throws IOException {
         if (objects != null) {
             final Iterator<S> it = objects.iterator();
             while (it.hasNext()) {
                 final Streamable s = it.next();
                 s.writeToStream(os);
     ...
     * @return a possibly truncated string
      */
     protected static String truncate(String str, final int maxLength) {
