 ...
 * with the page group. When a Tag Logical Element structured field is
 * associated with a page group, the parameters of the Tag Logical Element
 * structured field are inherited by all pages in the page group and by all
 * other page groups that are nested in the page group. The scope of a Tag
 * Logical Element is determined by its position with respect to other TLEs that
 * reference, or are embedded in, the same page or page group. The Tag Logical
 * Element structured field does not provide any presentation specifications and
 * therefore has no effect on the appearance of a document when it is presented.
 * <p/>
 */
public class TagLogicalElement extends AbstractTripletStructuredObject {
     ...
     */
    private final int tleID;

    /**
     * Construct a tag logical element with the name and value specified.
     *
     * @param name
     *            the name of the tag logical element
     * @param value
     *            the value of the tag logical element
     * @param tleID
     *            unique identifier for TLE within AFP stream
     */
    public TagLogicalElement(final String name, final String value,
            final int tleID) {
        this.name = name;
        this.value = value;
        this.tleID = tleID;
     ...
     */
    public void setAttributeQualifier(final int seqNumber, final int levNumber) {
    ...
    public void writeToStream(final OutputStream os) throws IOException {
        setFullyQualifiedName(FullyQualifiedNameTriplet.TYPE_ATTRIBUTE_GID,
                FullyQualifiedNameTriplet.FORMAT_CHARSTR, this.name);
        setAttributeValue(this.value);
        setAttributeQualifier(this.tleID, 1);

        final byte[] data = new byte[SF_HEADER.length];
        copySF(data, Type.ATTRIBUTE, Category.PROCESS_ELEMENT);

        final int tripletDataLength = getTripletDataLength();
        final byte[] l = BinaryUtils.convert(data.length + tripletDataLength
                - 1, 2);
        data[1] = l[0];
        data[2] = l[1];
        os.write(data);
