...
@Slf4j
 public class PropertyMaker implements Cloneable {
    ...
    private static final boolean IS_LOG_TRACE_ENABLED = log.isTraceEnabled();
     ...
     /**
     * Indicates whether the property is context-dependant and therefore can't
     * be cached.
     */
     ...
     *            The Constant ID of the property to be made.
      */
     public PropertyMaker(final int propId) {
         this.propId = propId;
     }
     ...
     *            a generic property maker.
      */
     public void useGeneric(final PropertyMaker generic) {
         ...
         if (generic.shorthands != null) {
             ...
             System.arraycopy(generic.shorthands, 0, this.shorthands, 0,
                    this.shorthands.length);
         }
     ...
     *            the PropertyMaker for the subproperty
      */
     public void addSubpropMaker(final PropertyMaker subproperty) {
     ...
     * @return The subproperty maker.
      */
     public PropertyMaker getSubpropMaker(final int subpropertyId) {
         throw new RuntimeException("Unable to add subproperties");
     }

     /**
      * Add a shorthand to this maker. Only an Integer is added to the shorthands
     * list. Later the Integers are replaced with references to the actual
     * shorthand property makers.
     * 
     * @param shorthand
     *            a property maker thar is that is checked for shorthand values.
      */
     public void addShorthand(final PropertyMaker shorthand) {
     ...
     *            the shorthand parser
      */
     public void setDatatypeParser(final ShorthandParser parser) {
     ...
     *            the default value
     * @param contextDep
     *            true when the value context dependent and must not be cached.
      */
     public void setDefault(final String defaultValue, final boolean contextDep) {
         this.defaultValue = defaultValue;
     ...
     *            the percent base (ex. LengthBase.FONTSIZE)
      */
     public void setPercentBase(final int percentBase) {
         this.percentBase = percentBase;
     }

     /**
      * Set the setByShorthand flag which only is applicable for subproperty
     * makers. It should be true for the subproperties which must be assigned a
     * value when the base property is assigned a attribute value directly.
     * 
     * @param setByShorthand
     *            true if this subproperty must be set when the base property is
     *            set
      */
     public void setByShorthand(final boolean setByShorthand) {
      ...
      * 
     * @param corresponding
     *            a corresponding maker where the isForcedCorresponding and
     *            compute methods are delegated to.
      */
     public void setCorresponding(final CorrespondingPropertyMaker corresponding) {
     ...
     * value specified, the absolute value is used. This is also true of the
     * inheritance priority (I think...) If the property is an "absolute"
     * property and it isn't specified, then we try to compute it from the
     * corresponding relative property: this happens in computeProperty.
     * 
     * @param propertyList
     *            the applicable property list
     * @param tryInherit
     ...
     *             if there is a problem evaluating the property
      */
     public Property findProperty(final PropertyList propertyList,
            final boolean tryInherit) throws PropertyException {
         Property p = null;

         if (IS_LOG_TRACE_ENABLED) {
             log.trace("PropertyMaker.findProperty: "
                    + FOPropertyMapping.getPropertyName(this.propId) + ", "
                    + propertyList.getFObj().getName());
         }

         if (this.corresponding != null
                && this.corresponding.isCorrespondingForced(propertyList)) {
             ...
             if (p == null) {
                 p = compute(propertyList);
             }
         }
         if (p == null && tryInherit) {
             // else inherit (if has parent and is inheritable)
             final PropertyList parentPropertyList = propertyList
     ...
     * 
     * @param subpropertyId
     *            The subproperty id of the property being retrieved. Is 0 when
     *            retrieving a base property.
     * @param propertyList
     ...
     *             if there is a problem evaluating the property
      */
     public Property get(final int subpropertyId,
            final PropertyList propertyList, final boolean tryInherit,
            final boolean tryDefault) throws PropertyException {
     ...
     * @return A boolean indicating whether this property is inherited.
     ...
     /**
      * This is used to handle properties specified as a percentage of some
     * "base length", such as the content width of their containing box.
     * Overridden by subclasses which allow percent specifications. See the
     * documentation on properties.xsl for details.
     * 
     * @param pl
     *            the PropertyList containing the property. (TODO: explain what
     *            this is used for, or remove it from the signature.)
     * @return an object implementing the PercentBase interface.
      * @throws PropertyException
     *             if there is a problem while evaluating the base property
      */
     ...
     *            SpaceProperty.
     * @param subpropertyId
     *            the id of the component whose value is to be returned. NOTE:
     *            this is only to ease porting when calls are made to
     *            PropertyList.get() using a component name of a compound
     *            property, such as get("space.optimum"). The recommended
     *            technique is: get("space").getOptimum(). Overridden by
     *            property maker subclasses which handle compound properties.
     * @return the Property containing the subproperty
      */
     public Property getSubprop(final Property p, final int subpropertyId) {
         final CompoundDatatype val = (CompoundDatatype) p.getObject();
         return val.getComponent(subpropertyId);
     }

     /**
      * Set a component in a compound property and return the modified compound
     * property object. This default implementation returns the original base
     * property without modifying it. It is overridden by property maker
     * subclasses which handle compound properties.
     * 
     * @param baseProperty
     *            The Property object representing the compound property, such
     *            as SpaceProperty.
     * @param subpropertyId
     *            The ID of the component whose value is specified.
     * @param subproperty
     *            A Property object holding the specified value of the component
     *            to be set.
     ...
     *             for invalid or inconsisten FO input
     ...
     public Property make(final PropertyList propertyList)
         ...
         final Property p = make(propertyList, this.defaultValue,
    ...
    public Property make(final PropertyList propertyList, final String value,
         ...
         try {
             ...
             if ("inherit".equals(value)) {
                 ...
                 if ((this.propId & Constants.COMPOUND_MASK) != 0) {
                     newProp = getSubprop(newProp, this.propId
                 ...
                 if (!isInherited() && log.isWarnEnabled()) {
                     ...
                     * be available)
                     */
                     final Property parentExplicit = propertyList
                            .getParentPropertyList().getExplicit(getPropId());
             ...
             if (newProp == null) {
                ...
                // Property.Maker
                 newProp = PropertyParser.parse(pvalue, new PropertyInfo(this,
                        propertyList));
             }
             if (newProp != null) {
             ...
             return newProp;
         } catch (final PropertyException propEx) {
             if (fo != null) {
     ...
     /**
      * Make a property value for a compound property. If the property value is
     * already partially initialized, this method will modify it.
     * 
     * @param baseProperty
     *            The Property object representing the compound property, for
     *            example: SpaceProperty.
     * @param subpropertyId
     *            The Constants ID of the subproperty (component) whose value is
     *            specified.
     * @param propertyList
     ...
     *             for invalid or inconsistent FO input
      */
     public Property make(final Property baseProperty, final int subpropertyId,
            final PropertyList propertyList, final String value, final FObj fo)
            throws PropertyException {
      ...
      *
      * @param propertyList
     *            the propertyList for which to convert
     * @param prop
     ...
     *             ...
      */
     public Property convertShorthandProperty(final PropertyList propertyList,
            final Property prop, final FObj fo) throws PropertyException {
         Property pret = convertProperty(prop, propertyList, fo);
         if (pret == null) {
             // If value is a name token, may be keyword or Enum
             final String sval = prop.getNCname();
             if (sval != null) {
                 // log.debug("Convert shorthand ncname " + sval);
                 pret = checkEnumValues(sval);
                 if (pret == null) {
                     /* Check for keyword shorthand values to be substituted. */
                     final String pvalue = checkValueKeywords(sval);
                     if (!pvalue.equals(sval)) {
                         // log.debug("Convert shorthand keyword" + pvalue);
                         // Substituted a value: must parse it
                         final Property p = PropertyParser.parse(pvalue,
                                new PropertyInfo(this, propertyList));
         ...
         if (pret != null) {
             /*
             * log.debug("Return shorthand value " + pret.getString() + " for "
             * + getPropName());
             */
         }
         return pret;
     ...
     /**
      * For properties that contain enumerated values. This method should be
     * overridden by subclasses.
     * 
     * @param value
     *            the string containing the property value
     * @return the Property encapsulating the enumerated equivalent of the input
     *         value
      */
     protected Property checkEnumValues(final String value) {
         if (this.enums != null) {
             final Property p = (Property) this.enums.get(value);
             return p;
         }
         return null;
     }
     ...
     /**
      * Return a String to be parsed if the passed value corresponds to a keyword
     * which can be parsed and used to initialize the property. For example, the
     * border-width family of properties can have the initializers "thin",
     * "medium", or "thick". The FOPropertyMapping file specifies a length value
     * equivalent for these keywords, such as "0.5pt" for "thin".
     * 
     * @param keyword
     *            the string value of property attribute.
     * @return a String containing a parseable equivalent or null if the passed
     *         value isn't a keyword initializer for this Property
      */
     protected String checkValueKeywords(final String keyword) {
         if (this.keywords != null) {
             final String value = (String) this.keywords.get(keyword);
             if (value != null) {
                 return value;
             }
         }
     ...
     /**
      * Return a Property object based on the passed Property object. This method
     * is called if the Property object built by the parser isn't the right type
     * for this property. It is overridden by subclasses.
     * 
     * @param p
     ...
     * @param fo
     *            The parent FO for the FO whose property is being made.
     * @return A Property of the correct type or null if the parsed value can't
     *         be converted to the correct type.
     * @throws PropertyException
     *             for invalid or inconsistent FO input
      */
     protected Property convertProperty(final Property p,
     ...
     * routine should be overridden to attempt to set them based upon the other
     * methods. For example, colors may be specified using an RGB model, or they
     * may be specified using an NCname.
     * 
     * @param p
     *            property whose datatype should be converted
     * @param propertyList
     *            collection of properties. (TODO: explain why this is needed,
     *            or remove it from the signature.)
     * @param fo
     *            The parent FO for the FO whose property is being made. why
     *            this is needed, or remove it from the signature).
     * @return an Property with the appropriate datatype used
      * @throws PropertyException
     *             for invalid or inconsistent input
      */
     protected Property convertPropertyDatatype(final Property p,
            final PropertyList propertyList, final FObj fo)
            throws PropertyException {
         return null;
     }
     ...
     *         specified to compute the value.
     * @throws PropertyException
     *             for invalid or inconsistent FO input
      */
     ...
     * should return the Property, if any, that is parsed from any shorthand
     * properties that affect this property. This method expects to be
     * overridden by subclasses. For example, the border-right-width property
     * could be set implicitly from the border shorthand property, the
     * border-width shorthand property, or the border-right shorthand property.
     * This method should be overridden in the appropriate subclass to check
     * each of these, and return an appropriate border-right-width Property
     * object.
     * 
     * @param propertyList
     ...
     * @return the Property, if found, the correspons, otherwise, null
      * @throws PropertyException
     *             if there is a problem while evaluating the shorthand
      */
     public Property getShorthand(final PropertyList propertyList)
            throws PropertyException {
         if (this.shorthands == null) {
             return null;
         }
         ...
         for (int i = 0; i < n && this.shorthands[i] != null; ++i) {
             ...
             if (prop != null) {
                 final ShorthandParser parser = shorthand.datatypeParser;
                 final Property p = parser.getValueForProperty(getPropId(),
     ...
     * subproperty makers of the generic compound makers. {@inheritDoc}
      */
     @Override
     public Object clone() {
         try {
