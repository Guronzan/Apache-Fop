...
@Slf4j
 public class FontShorthandProperty extends ListProperty {
     ...
     public static class Maker extends PropertyMaker {
         ...
         private static final int[] PROP_IDS = { Constants.PR_FONT_SIZE,
                Constants.PR_FONT_FAMILY, Constants.PR_LINE_HEIGHT,
                Constants.PR_FONT_STYLE, Constants.PR_FONT_VARIANT,
         ...
         *            ID of the property for which Maker should be created
         ...
         public Property make(final PropertyList propertyList, String value,
             ...
             try {
                 final FontShorthandProperty newProp = new FontShorthandProperty();
                 newProp.setSpecifiedValue(value);

                 final String specVal = value;
                 Property prop = null;
                 if ("inherit".equals(specVal)) {
                     /*
                     * fill the list with the individual properties from the
                     * parent
                     */
                     ...
                     if (prop == null) {
                         /*
                         * not an enum: value should consist at least of
                         * font-size and font-family separated by a space mind
                         * the possible spaces from quoted font-family names
                         */
                         int spaceIndex = value.indexOf(' ');
                         int quoteIndex = value.indexOf('\'') == -1 ? value
                                .indexOf('\"') : value.indexOf('\'');
                                 if (spaceIndex == -1 || quoteIndex != -1
                                && spaceIndex > quoteIndex) {
                                     /*
                             * no spaces or first space appears after the first
                             * single/double quote, so malformed value string
                             */
                                     throw new PropertyException(
                                    "Invalid property value: " + "font=\""
                                            + value + "\"");
                                 }
                         ...
                         * single/double quote, so extract the individual
                         * properties
                         */
                             ...
                             */
                                     if (commaIndex == -1) {
                                         /*
                                 * no list, just a single name (or first name in
                                 * the list)
                                 */
                                         if (quoteIndex != -1) {
                                             /*
                                     ...
                                     */
                                         ...
                                         prop = m.make(propertyList,
                                        specVal.substring(fromIndex), fo);
                                     ...
                                     * in the comma-separated list fromIndex =
                                     * index of the first quote
                                     */
                                             fromIndex = quoteIndex;
                                             quoteIndex = -1;
                                         } else {
                                             fromIndex = value.lastIndexOf(' ',
                                            commaIndex) + 1;
                                         }
                                         commaIndex = -1;
                                     }
                                 ...
                                 final String fontSize = value.substring(0,
                                         slashIndex == -1 ? value.length() : slashIndex);
                                 m = FObj.getPropertyMakerFor(PROP_IDS[0]);
                                 prop = m.make(propertyList, fontSize, fo);
                                 /*
                         * need to make sure subsequent call to
                         * LineHeightPropertyMaker.make() doesn't generate the
                         ...
                         */
                                 ...
                                 if (fromIndex != 0) {
                                     ...
                                     do {
                                         toIndex = spaceIndex == -1 ? value.length()
                                        : spaceIndex;
                                         String val = value
     ...
     private void addProperty(final Property prop, final int pos) {
