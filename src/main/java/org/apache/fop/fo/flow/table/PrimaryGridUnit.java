...
/**
 * This class represents a primary grid unit of a spanned cell. This is the
 * "before-start" (top-left, usually) grid unit of the span.
 */
public class PrimaryGridUnit extends GridUnit {
    ...
    private final boolean isSeparateBorderModel;
    private final int halfBorderSeparationBPD;
     ...
     */
    PrimaryGridUnit(final TableCell cell, final int colIndex) {
        super(cell, 0, 0);
        this.colIndex = colIndex;
        this.isSeparateBorderModel = cell.getTable().isSeparateBorderModel(); // TODO
        this.halfBorderSeparationBPD = cell.getTable().getBorderSeparation()
                .getBPD().getLength().getValue() / 2; // TODO
    }
    ...
    /**
     * Returns the widths of the border-before and -after for this cell. In the
     * separate border model the border-separation is included. In the
     * collapsing model only half of them is counted, since the other halves
     * belong to the neighbouring cells; also, the returned value is the maximum
     * of the segments of each applicable grid unit.
     *
    ...
    /**
     * Returns the width of the before-border for the given row-span of this
     * cell. In the separate border model half of the border-separation is
     * included. In the collapsing model only half of the border is counted,
     * since the other half belongs to the preceding cell; also, the returned
     * value is the maximum of the segments of each applicable grid unit.
     *
     * @param rowIndex
     *            index of the span for which the border must be computed,
     *            0-based
     * @param which
     ...
     */
    public int getBeforeBorderWidth(final int rowIndex, final int which) {
    ...
    /**
     * Returns the width of the before-after for the given row-span of this
     * cell. In the separate border model half of the border-separation is
     * included. In the collapsing model only half of the border is counted,
     * since the other half belongs to the following cell; also, the returned
     * value is the maximum of the segments of each applicable grid unit.
     *
     * @param rowIndex
     *            index of the span for which the border must be computed,
     ...
     */
    public int getAfterBorderWidth(final int rowIndex, final int which) {
        if (this.isSeparateBorderModel) {
            if (getCell() == null) {
                return 0;
            } else {
     ...
     * See {@link #getAfterBorderWidth(int, int)}.
     *
     * @param which
     *            one of {@link ConditionalBorder#NORMAL},
     *            {@link ConditionalBorder#LEADING_TRAILING} or
     ...
     * @return the after border width
     */
    public int getAfterBorderWidth(final int which) {
    ...
    public int getContentLength() {
        if (this.contentLength < 0) {
            this.contentLength = ElementListUtils
                    .calcContentLength(this.elements);
        }
    ...
    /**
     * Returns the index of the row this grid unit belongs to. This is the
     * index, in the enclosing table part, of the first row spanned by the cell.
     * Note that if the table has several table-body children, then the index
     * grows continuously across them; they are considered to form one single
     * part, the "body of the table".
     *
    ...
    /**
     * Returns the widths of the start- and end-borders of the span this grid
     * unit belongs to.
     *
     * @return a two-element array containing the widths of the start-border
     *         then the end-border
     */
    public int[] getStartEndBorderWidths() {
        ...
        if (getCell() == null) {
            ...
            widths[0] = getCell().getCommonBorderPaddingBackground()
    ...
    /**
     * Creates a cellLM for the corresponding table-cell. A new one must be
     * created for each new static-content (TODO).
     */
    ...
    /**
     * Returns the strength of the keep constraint if the first child block (or
     * its descendants) of this cell has keep-with-previous.
     *
     ...
     *            the keep strength
     */
    public void setKeepWithPrevious(final Keep keep) {
        this.keepWithPrevious = keep;
    }

    /**
     * Returns the strength of the keep constraint if the last child block (or
     * its descendants) of this cell has keep-with-next.
     *
     ...
     *            the keep strength
     */
    public void setKeepWithNext(final Keep keep) {
        this.keepWithNext = keep;
    }

    /**
     * Returns the class of the before break for the first child element of this
     * cell.
     *
    ...
    /**
     * Returns the class of the before after for the last child element of this
     * cell.
     *
     * @return one of {@link Constants#EN_AUTO}, {@link Constants#EN_COLUMN},
     *         {@link Constants#EN_PAGE}, {@link Constants#EN_EVEN_PAGE},
