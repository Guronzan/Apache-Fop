...
public class PDFShading extends PDFObject {
    ...
    /**
     * Optional: A flag whether or not to filter the shading function to prevent
     * aliasing artifacts. Default is false.
     */
    protected boolean antiAlias = false;

    /**
     * Optional for Type 1: Array of four numbers, xmin, xmax, ymin, ymax.
     * Default is [0 1 0 1] Optional for Type 2: An array of two numbers between
     * which the blend varies between start and end points. Default is 0, 1.
    ...
    /**
     * Required for Type 1, 2, and 3: The object of the color mapping function
     * (usually type 2 or 3). Optional for Type 4,5,6, and 7: When it's nearly
     * the same thing.
     */
    protected PDFFunction function = null;

    /**
     * Required for Type 2: An Array of four numbers specifying the starting and
     * ending coordinate pairs Required for Type 3: An Array of six numbers
     * [x0,y0,r0,x1,y1,r1] specifying the centers and radii of the starting and
     * ending circles.
     */
    protected List coords = null;

    /**
     * Required for Type 2+3: An Array of two boolean values specifying whether
     * to extend the start and end colors past the start and end points,
     * respectively. Default is false, false.
     */
    protected List extend = null;

    /**
     * Required for Type 4,5,6, and 7: Specifies the number of bits used to
     * represent each vertex coordinate. Allowed to be 1,2,4,8,12,16,24, or 32.
     */
    protected int bitsPerCoordinate = 0;

    /**
     * Required for Type 4,5,6, and 7: Specifies the number of bits used to
     * represent the edge flag for each vertex. Allowed to be 2,4,or 8, while
     * the Edge flag itself is allowed to be 0,1 or 2.
     */
    protected int bitsPerFlag = 0;

    /**
     * Required for Type 4,5,6, and 7: Array of Doubles which specifies how to
     * decode coordinate and color component values. Each type has a differing
     * number of decode array members, so check the spec. Page 303 in PDF Spec
     * 1.3
     */
    ...
    /**
     * Required for Type 4,5,6, and 7: Specifies the number of bits used to
     * represent each color coordinate. Allowed to be 1,2,4,8,12, or 16
     */
    protected int bitsPerComponent = 0;

    /**
     * Required for Type 5:The number of vertices in each "row" of the lattice;
     ...
     */
    public PDFShading(final int theShadingType,
     ...
     *
     * @param name
     *            the name of the shading pattern. Can be anything without
    ...
    /**
     * represent as PDF. Whatever the shadingType is, the correct representation
     * spits out. The sets of required and optional attributes are different for
     * each type, but if a required attribute's object was constructed as null,
     * then no error is raised. Instead, the malformed PDF that was requested by
     * the construction is dutifully output. This policy should be reviewed.
     *
    ...
    public String toPDFString() {
        ...
        if (this.background != null) {
            ...
            for (tempInt = 0; tempInt < vectorSize; tempInt++) {
                p.append(PDFNumber.doubleOut((Double) this.background
                        .get(tempInt)) + " ");
            }
            p.append("] \n");
        }

        if (this.bBox != null) { // I've never seen an example, so I guess this
            // is right.
            p.append("/BBox [ ");
        ...
        if (this.shadingType == 1) { // function based shading
            ...
            if (this.matrix != null) {
                ...
                for (tempInt = 0; tempInt < vectorSize; tempInt++) {
                    p.append(PDFNumber.doubleOut((Double) this.matrix
                            .get(tempInt)) + " ");
                }
                p.append("] \n");
            }
        ...
        } else if (this.shadingType == 2 || this.shadingType == 3) { // 2 is
            // axial
            // shading
            // (linear
            // gradient)
            ...
            if (this.coords != null) {
                ...
                for (tempInt = 0; tempInt < vectorSize; tempInt++) {
                    p.append(PDFNumber.doubleOut((Double) this.coords
                            .get(tempInt)) + " ");
                }
                p.append("] \n");
            }
            ...
            if (this.extend != null) {
                ...
                for (tempInt = 0; tempInt < vectorSize; tempInt++) {
                    p.append(this.extend.get(tempInt) + " ");
                }
            ...
            if (this.function != null) {
                ...
                || this.shadingType == 7) { // 4:Free-form Gouraud-shaded
            // triangle meshes
            ...
            if (this.decode != null) {
                ...
                for (tempInt = 0; tempInt < vectorSize; tempInt++) {
                    p.append(this.decode.get(tempInt) + " ");
                }
        ...
        } else if (this.shadingType == 5) { // Lattice Free form gouraud-shaded
            // triangle mesh
    ...
    protected boolean contentEquals(final PDFObject obj) {
        ...
        final PDFShading shad = (PDFShading) obj;
