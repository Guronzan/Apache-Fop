...
public class PDFFontDescriptor extends PDFDictionary {
     ...
     */
    public PDFFontDescriptor(final String basefont, final int ascent,
            final int descent, final int capHeight, final int flags,
            final PDFRectangle fontBBox, final int italicAngle, final int stemV) {
        super();
     ...
     * 
     * @param avgWidth
     *            The average width of characters in this font. The default
     *            value is 0.
     * @param maxWidth
     *            The maximum width of characters in this font. The default
     *            value is 0.
     * @param missingWidth
     *            missing width
     * @param leading
     *            the desired spacing between lines of text. The default value
     *            is 0.
     * @param stemH
     *            The vertical width of the dominant horizontal stems of glyphs
     *            in the font. The default value is 0.
     * @param xHeight
     *            The y-coordinate of the top of flat non-ascending lowercase
     ...
     *
     * @param subtype
     *            the font type defined in the font stream
     * @param fontfile
     *            the stream containing an embedded font
     */
    public void setFontFile(final FontType subtype,
            final AbstractPDFStream fontfile) {
     ...
     *            the CIDSet stream
     */
    public void setCIDSet(final AbstractPDFStream cidSet) {
        put("CIDSet", cidSet);
    }
     ...
     * p.append(capHeight); p.append("\n/Ascent "); p.append(ascent);
     * p.append("\n/Descent "); p.append(descent); p.append("\n/ItalicAngle ");
     * p.append(italicAngle); p.append("\n/StemV "); p.append(stemV); //
     * optional fields if (stemH != 0) { p.append("\n/StemH "); p.append(stemH);
     * } if (xHeight != 0) { p.append("\n/XHeight "); p.append(xHeight); } if
     * (avgWidth != 0) { p.append("\n/AvgWidth "); p.append(avgWidth); } if
     * (maxWidth != 0) { p.append("\n/MaxWidth "); p.append(maxWidth); } if
     * (missingWidth != 0) { p.append("\n/MissingWidth ");
     * p.append(missingWidth); } if (leading != 0) { p.append("\n/Leading ");
     * p.append(leading); } if (fontfile != null) { if (subtype ==
     * FontType.TYPE1) { p.append("\n/FontFile "); } else {
     * p.append("\n/FontFile2 "); } p.append(fontfile.referencePDF()); } if
     * (getCIDSet() != null) { p.append("\n/CIDSet ");
     * p.append(getCIDSet().referencePDF()); } // charSet for subset fonts //
     * not yet implemented // CID optional field fillInPDF(p);
     * p.append(" >>\nendobj\n"); return p.toString(); }
     */
