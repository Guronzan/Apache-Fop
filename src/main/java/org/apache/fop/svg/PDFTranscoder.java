 ...
 public class PDFTranscoder extends AbstractFOPTranscoder {
     ...
     *                if an error occured while transcoding
     ...
     protected void transcode(final Document document, final String uri,
         ...
         final float widthInPt = UnitProcessor.userSpaceToSVG(this.width,
                SVGLength.SVG_LENGTHTYPE_PT, UnitProcessor.HORIZONTAL_LENGTH,
                uctx);
         final int w = (int) (widthInPt + 0.5);
         final float heightInPt = UnitProcessor.userSpaceToSVG(this.height,
                SVGLength.SVG_LENGTHTYPE_PT, UnitProcessor.HORIZONTAL_LENGTH,
                uctx);
