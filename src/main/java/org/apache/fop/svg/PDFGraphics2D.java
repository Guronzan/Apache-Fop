...
public class PDFGraphics2D extends AbstractGraphics2D implements
    ...
    protected void applyColor(final Color col, final boolean fill) {
        ...
        if (col instanceof ColorExt) {
            final PDFColor currentColour = new PDFColor(this.pdfDoc, col);
            this.currentStream.write(currentColour.getColorSpaceOut(fill));
        } else if (c.getColorSpace().getType() == ColorSpace.TYPE_RGB) {
            final PDFColor currentColour = new PDFColor(c.getRed(),
            ...
            final PDFColor currentColour = new PDFColor(c);
            this.currentStream.write(currentColour.getColorSpaceOut(fill));
        } else if (c.getColorSpace().getType() == ColorSpace.TYPE_2CLR) {
     ...
     *         be rasterized
     */
    protected boolean applyPaint(Paint paint, final boolean fill) {
        preparePainting();
        ...
        if (paint instanceof LinearGradientPaint) {
            ...
            for (int count = 0; count < cols.length; count++) {
                ...
                final PDFColor color1 = new PDFColor(c1.getRed(),
        ...
        if (paint instanceof RadialGradientPaint) {
            ...
            for (final Color cc : cols) {
                ...
                someColors.add(new PDFColor(cc.getRed(), cc.getGreen(), cc
                        .getBlue()));
            }
    ...
    private boolean createPattern(final PatternPaint pp, final boolean fill) {
        ...
        final Rectangle2D gnBBox = gn.getBounds();
    ...
    protected boolean applyUnknownPaint(final Paint paint, final Shape shape) {
        ...
        this.currentStream.write("q\n");
        writeClip(shape);
        this.currentStream.write("" + usrW + " 0 0 " + -usrH + " " + usrX + " "
    ...
    protected boolean isMultiByteFont(final String name) {
        ...
        final org.apache.fop.fonts.Typeface f = (org.apache.fop.fonts.Typeface) this.fontInfo
                .getFonts().get(name);
